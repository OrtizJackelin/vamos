<?php 
    ...
    //var_dump($_POST);
    extract($_POST);
    $consulta = "INSERT INTO reseña
    SET id_usuario = ?, id_publicacion = ?, comentario = ?, calificacion = ?";

    $sentencia = $conexion->stmt_init();
    if(!$sentencia->prepare($consulta)){
        echo "Error preparando la consulta para insertar datos ";
    } else{
        $sentencia ->bind_param("ssss", $idUsuario,  $idPublicacion, $comentario, $calificacion);
        $sentencia->execute();
